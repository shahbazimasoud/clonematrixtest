...
app.get("/api/vpn-proxy/status", authenticateToken, (req, res) => {
  try {
    const db = readDb();
    if (!db.vpnProxySettings) {
      db.vpnProxySettings = {
        routeProtectionEnabled: true,
        panelIp: req.headers.host || "127.0.0.1",
        gatewayIp: "10.0.0.1",
        targetServerIp: "185.220.101.5",
        targetServerName: "Primary Gateway Node",
        protocols: {
          pptp: { enabled: true, status: "running", port: 1723, subnet: "10.8.0.0/24", clientsCount: 3, packageInstalled: true },
          l2tp: { enabled: true, status: "running", port: 1701, ipsecKey: "MatrixPsk2026!", subnet: "10.9.0.0/24", clientsCount: 5, packageInstalled: true },
          sstp: { enabled: true, status: "running", port: 443, certType: "LetsEncrypt", subnet: "10.10.0.0/24", clientsCount: 8, packageInstalled: true },
          socks5: { enabled: true, status: "running", port: 1080, authRequired: true, clientsCount: 12, packageInstalled: true },
          httpProxy: { enabled: false, status: "stopped", port: 8080, authRequired: true, clientsCount: 0, packageInstalled: false },
          shadowsocks: { enabled: false, status: "stopped", port: 8388, method: "aes-256-gcm", clientsCount: 0, packageInstalled: false }
        },
        routeBypassList: ["127.0.0.1", "10.0.0.0/8", "192.168.1.0/24", req.headers.host?.split(':')[0] || "localhost"]
      };
      writeDb(db);
    }
    return res.json(db.vpnProxySettings);
  } catch (err: any) {
    return res.status(500).json({ error: err.message });
  }
});

app.post("/api/vpn-proxy/install-packages", authenticateToken, checkPermission(["Owner", "Super Admin", "Admin"]), (req, res) => {
  try {
    const { protocols } = req.body; // array of protocol names e.g. ['sstp', 'l2tp', 'pptp', 'socks5']
    const db = readDb();
    if (!db.vpnProxySettings) db.vpnProxySettings = {};
    if (!db.vpnProxySettings.protocols) db.vpnProxySettings.protocols = {};

    const targetProtocols = Array.isArray(protocols) ? protocols : ['pptp', 'l2tp', 'sstp', 'socks5'];
    const logs: string[] = [];
...
app.post("/api/vpn-proxy/configure-protocol", authenticateToken, checkPermission(["Owner", "Super Admin", "Admin"]), (req, res) => {
  try {
    const { protocol, config } = req.body;
    if (!protocol || !config) return res.status(400).json({ error: "protocol and config required" });

    const db = readDb();
    if (!db.vpnProxySettings) db.vpnProxySettings = { protocols: {} };
    if (!db.vpnProxySettings.protocols) db.vpnProxySettings.protocols = {};

    db.vpnProxySettings.protocols[protocol] = {
      ...(db.vpnProxySettings.protocols[protocol] || {}),
      ...config
    };
...
app.post("/api/vpn-proxy/service-action", authenticateToken, checkPermission(["Owner", "Super Admin", "Admin"]), (req, res) => {
  try {
    const { protocol, action } = req.body; // action: start | stop | restart
    if (!protocol || !action) return res.status(400).json({ error: "protocol and action required" });

    const db = readDb();
    if (!db.vpnProxySettings?.protocols?.[protocol]) {
      return res.status(404).json({ error: "Protocol not found" });
    }

    const target = db.vpnProxySettings.protocols[protocol];
...
app.get("/api/vpn-proxy/users", authenticateToken, (req, res) => {
  try {
    const db = readDb();
    if (!db.vpnProxyUsers) {
      db.vpnProxyUsers = [
...
app.post("/api/vpn-proxy/users", authenticateToken, checkPermission(["Owner", "Super Admin", "Admin"]), (req, res) => {
  try {
    const { username, password, protocols, assignedIp } = req.body;
    if (!username || !password) return res.status(400).json({ error: "Username and password required" });

    const db = readDb();
    if (!db.vpnProxyUsers) db.vpnProxyUsers = [];

    const existingIndex = db.vpnProxyUsers.findIndex((u: any) => u.username === username);
    const newUser = {
      id: existingIndex >= 0 ? db.vpnProxyUsers[existingIndex].id : "vpu_" + Date.now(),
      username,
      password,
      protocols: Array.isArray(protocols) ? protocols : ["pptp", "l2tp", "sstp", "socks5"],
      assignedIp: assignedIp || "Dynamic",
      active: true,
      usage: "0 B"
    };

    if (existingIndex >= 0) {
      db.vpnProxyUsers[existingIndex] = newUser;
    } else {
      db.vpnProxyUsers.push(newUser);
    }
...
app.delete("/api/vpn-proxy/users/:id", authenticateToken, checkPermission(["Owner", "Super Admin", "Admin"]), (req, res) => {
  try {
    const { id } = req.params;
    const db = readDb();
    if (db.vpnProxyUsers) {
...
app.post("/api/vpn-proxy/route-protection", authenticateToken, checkPermission(["Owner", "Super Admin", "Admin"]), (req, res) => {
  try {
    const { enabled, panelIp, gatewayIp, customBypassRoutes } = req.body;
    const db = readDb();
    if (!db.vpnProxySettings) db.vpnProxySettings = {};

    db.vpnProxySettings.routeProtectionEnabled = !!enabled;
    if (panelIp) db.vpnProxySettings.panelIp = panelIp;
    if (gatewayIp) db.vpnProxySettings.gatewayIp = gatewayIp;
    if (Array.isArray(customBypassRoutes)) db.vpnProxySettings.routeBypassList = customBypassRoutes;
...
app.post("/api/vpn-proxy/test-panel-route", authenticateToken, (req, res) => {
  try {
    const db = readDb();
    const isProtected = db.vpnProxySettings?.routeProtectionEnabled ?? true;
    return res.json({
      success: true,
      reachable: true,
      latencyMs: Math.floor(Math.random() * 15) + 5,
      routeProtectionStatus: isProtected ? "Active (Bypass Active)" : "Inactive",
      panelHost: req.headers.host || "localhost",
      message: "ارتباط مستقیم با پنل مدیریت بدون قطعی و مستقل از ترافیک VPN برقرار است."
    });
  } catch (err: any) {
    return res.status(500).json({ error: err.message });
  }

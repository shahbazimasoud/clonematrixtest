...
export default function WallpaperTab({
  ...
  // Fetch wallpapers, branding config, and security captcha settings
  ...
  const handleSaveBranding = async () => {
    ...
    try {
      ...
      if (resBranding.ok) {
        ...
        const errData = await resBranding.json();
    ...
    <div className="space-y-6 animate-fadeIn" id="wallpaper-hub-container">
      ...
      <div className="pt-6 border-t border-white/10 space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="md:col-span-2 spatial-glass rounded-2xl p-5 border border-white/10 bg-white/5 space-y-4" id="section-auth-policy-config">
            {/* Header & Badges */}
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-white/10 pb-3">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    <h4 className="text-xs font-bold text-white uppercase tracking-wider">
                      {loc('کنترل دسترسی و سیاست ورود کاربران (Local / Active Directory)', 'User Login Source & Authentication Policy')}
                    </h4>
                    <span className="text-[10px] font-mono px-1.5 py-0.5 rounded bg-white/5 border border-white/5 text-slate-400">
                      password.yaml
                    </span>
              ...
              <div className="flex items-center gap-2 self-start sm:self-auto">
                {branding.authPolicy === 'ldap_only' ? (
                  <span className="px-2.5 py-1 rounded-full text-[11px] font-bold bg-amber-500/15 text-amber-300 border border-amber-500/30 flex items-center gap-1">
                    ...
                    {loc('فقط اکتیو دایرکتوری (لوکال مسدود)', 'AD Only (Local Blocked)')}
                  </span>
                ) : (
                  <span className="px-2.5 py-1 rounded-full text-[11px] font-bold bg-emerald-500/15 text-emerald-300 border border-emerald-500/30 flex items-center gap-1">
                    <Check className="w-3 h-3" />
                    {loc('هردو (لوکال + اکتیو دایرکتوری)', 'Both (Local + AD)')}
                  </span>
                )}
              </div>
            </div>

            {/* Authentication Source Selection Options (2 Options) */}
            ...
            <div className="rounded-xl p-3.5 bg-black/40 border border-white/10 space-y-2">
              <div className="flex items-center justify-between">
